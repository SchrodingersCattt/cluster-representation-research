...
def write_single_system(system_dir: Path, coords_cart: np.ndarray, box: np.ndarray, atom_types: np.ndarray, prop: float) -> None:
    ...
    np.save(set_dir / "force.npy", np.zeros((1, coords_cart.shape[0], 3), dtype=np.float64))


def main() -> None:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    bundle = np.load(NPZ_PATH, allow_pickle=True)
    kept = json.loads(FILTERED_INDEX.read_text(encoding="utf-8"))["kept"]

    for rec in kept:
        if not rec["has_exp_det_velocity"]:
            continue
        idx = rec["index"]
        refcode = rec["refcode"]
        prop = float(rec["exp_det_velocity_m_s"])
    ...
    print(f"Wrote exp validation systems to {OUT_DIR}")
